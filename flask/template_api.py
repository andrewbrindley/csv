...
from field_enricher import enrich_fields_async, _field_hash
...
def api_create_template():
    ...
    # Fire AI enrichment for all fields in the background (non-blocking)
    template_label = template_doc.get("templateLabel", template_key)
    enrich_fields_async(fields, template_key, template_label)
...
def api_update_template(template_key):
    ...
    # Delta enrichment: only enrich fields whose definition changed
    if "fields" in body:
        new_fields = body["fields"]
        # Compute hashes of old fields to find the delta
        old_hashes = {
            _field_hash(template_key, f)
        ...
        changed_hashes = new_hashes - old_hashes  # only new/modified fields
        if changed_hashes:
            template_label = update_data.get("templateLabel") or existing.get("templateLabel", template_key)
            enrich_fields_async(new_fields, template_key, template_label, changed_hashes=changed_hashes)
            print(f"ENRICH: queued {len(changed_hashes)} changed field(s) for {template_key}")
        else:
            print(f"ENRICH: no field changes detected for {template_key}, skipping enrichment")
    ...
    return jsonify({
        ...
        "template": updated,
        "enrichmentStatus": "queued" if ("fields" in body and changed_hashes) else "unchanged",
    })
