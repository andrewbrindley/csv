...
from field_enricher import apply_enrichments_to_fields
...
def get_templates(tenant_id: str = None) -> dict:
    ...
    # Inject AI enrichments (synonyms + value hints) into all fields —
    # single bulk MongoDB query per template, O(1) lookup from cached hashes.
    for t_key, t_def in final_templates.items():
        fields = t_def.get("fields", [])
        if fields:
            t_def["fields"] = apply_enrichments_to_fields(fields, t_key)
