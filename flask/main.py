...
# ---------------------------------------------------------------------------
# Maximum number of ERRORED rows sent to the AI cleaning pass per request.
# At ~$0.002 per row (gpt-4o), 200 rows ≈ $0.40 max per clean call.
...
def seed_tenants():
    """Seed default tenants if the collection is empty."""
    coll = get_tenants_collection()
    if coll is None:
        return
    if coll.count_documents({}) == 0:
        print("TENANTS: Seeding default tenants...")
        tenants = [
            {"tenantId": "acme-corp", "name": "Acme Corp", "createdAt": datetime.utcnow()},
            {"tenantId": "globex", "name": "Globex Corp", "createdAt": datetime.utcnow()},
            {"tenantId": "stark-ind", "name": "Stark Industries", "createdAt": datetime.utcnow()},
        ]
        coll.insert_many(tenants)
        print(f"TENANTS: Inserted {len(tenants)} default tenants.")
...
swagger_template = {
    "info": {
        ...
        "description": "API documentation for the CSV data importing tool.\n\n"
                       "**Quick Start:** Click **Authorize** above, enter `admin-1` in "
                       "the X-User-ID field, then close. All endpoints will work.",
        "version": "1.0.0"
    },
    "securityDefinitions": {
        "Bearer": {
            ...
            "name": "Authorization",
            "in": "header",
            "description": "Enter 'Bearer <token>' — an Auth0 JWT or sk_live_ API key."
        },
        "UserID": {
            "type": "apiKey",
            "name": "X-User-ID",
            "in": "header",
            "description": "Dev shortcut: enter a userId like 'admin-1' to authenticate."
        }
    },
    "security": [
        {"Bearer": []},
...
swagger_config = {
    ...
    "swagger_ui": True,
    "specs_route": "/apidocs-internal/",  # Hidden — our custom /apidocs/ overrides
}
...
def custom_swagger_ui():
    """Single Swagger UI page — auto-authenticated, no Authorize button needed."""
    return '''<!DOCTYPE html>
<html>
<head>
    <title>CSV Import API</title>
...
def auth_me():
    """
    Return the authenticated user's profile, tenant roles, and accessible tenants.
    Called by the React frontend after login.
    ---
    tags:
      - Auth
    responses:
      200:
        description: User profile, tenant roles, and accessible tenants.
      401:
        description: Not authenticated.
    """
    user = request.user
    user_tenants = user.get("tenants", [])
    is_super = user.get("isSuperAdmin", False) or user.get("userId") == "admin-1"

    # Build list of accessible tenants with names
    tcoll = get_tenants_collection()
    all_tenants = []
    if tcoll is not None:
        all_tenants = list(tcoll.find({}, {"_id": 0, "tenantId": 1, "name": 1}))

    if is_super:
        accessible = all_tenants
    else:
        accessible_ids = {t.get("tenantId") for t in user_tenants}
        accessible = [t for t in all_tenants if t.get("tenantId") in accessible_ids]
...
def admin_list_users():
    """List all users who have access to a specific tenant.
    ---
    tags:
      - Admin
    parameters:
    ...
    responses:
      200:
        description: List of users with their roles in this tenant.
    """
    tenant_id = request.args.get("tenantId")
    coll = get_users_collection()
    if coll is None:
    ...
    # Find users who have this tenant in their tenants array
    users = list(coll.find(
        {"tenants.tenantId": tenant_id},
        {"_id": 0, "userId": 1, "name": 1, "email": 1, "picture": 1, "tenants": 1, "isSuperAdmin": 1}
    ))

    # Flatten: include only the role for this tenant
    result = []
    for u in users:
        role = None
...
def admin_update_user_role():
    ...
    tenant_id = body.get("tenantId") or request.args.get("tenantId")
    target_user_id = body.get("userId")
    new_role = body.get("role", "").upper()

    if new_role not in [ROLE_ADMIN, ROLE_EDITOR, ROLE_VIEWER]:
        return jsonify({"error": f"Invalid role '{new_role}'. Must be ADMIN, EDITOR, or VIEWER."}), 400

    # Prevent self-demotion (admins locking themselves out)
    ...
    if current_user_id and current_user_id == target_user_id:
        return jsonify({"error": "You cannot change your own role. Ask another admin to do this."}), 400

    coll = get_users_collection()
    if coll is None:
        return jsonify({"error": "Database unavailable"}), 500

    target = coll.find_one({"userId": target_user_id})
    if not target:
    ...
    if not found:
        tenants.append({"tenantId": tenant_id, "role": new_role})

    coll.update_one({"userId": target_user_id}, {"$set": {"tenants": tenants}})
    return jsonify({"message": f"User '{target_user_id}' is now {new_role} in '{tenant_id}'."})


@app.route("/api/admin/users/invite", methods=["POST"])
@require_role(ROLE_ADMIN)
def admin_invite_user():
    """Add an existing user to a tenant with a specific role, or create a placeholder.
    ---
    tags:
      - Admin
    parameters:
    ...
    new_role = body.get("role", "VIEWER").upper()
    ...
    if user:
        ...
        for t in tenants:
            if t.get("tenantId") == tenant_id:
                return jsonify({"message": f"User '{email}' already has access to '{tenant_id}'."}), 200
        tenants.append({"tenantId": tenant_id, "role": new_role})
        coll.update_one({"userId": user["userId"]}, {"$set": {"tenants": tenants}})
        return jsonify({"message": f"User '{email}' added to '{tenant_id}' as {new_role}."})

    # Create placeholder user (will be linked when they first log in via Auth0)
    placeholder = {
        "userId": f"pending:{email}",
        "name": email.split("@")[0],
        "email": email,
        "picture": "",
        "tenants": [{"tenantId": tenant_id, "role": new_role}],
        "isSuperAdmin": False,
        "createdAt": datetime.utcnow(),
    }
    coll.insert_one(placeholder)
    return jsonify({"message": f"Invited '{email}' to '{tenant_id}' as {new_role}. They will be activated on first Auth0 login."})
...
GLOBAL_SYNONYMS: dict[str, list[str]] = {
    ...
    "personnumber": ["personno", "personnum", "empno", "employeeno", "candidateno",
                     ...
                     "patientid", "patientno", "patientnum", "ptid", "ptno"],
...
# ---------------------------------------------------------------------------
# Universal string sanitiser — called before every string cleaner.
# Strips invisible/control unicode, decodes HTML entities, collapses spaces.
# ---------------------------------------------------------------------------
import html

_INVISIBLE_CHARS = re.compile(r"[\u200b\u200c\u200d\u200e\u200f\u00ad\ufeff\u00a0]+")

def _clean_string_universal(s: str) -> str:
    """Strip invisible unicode, decode HTML entities, collapse whitespace."""
    if not s:
        return s
    # Decode HTML entities: &amp; → &, &lt; → <, &nbsp; → space, etc.
    s = html.unescape(s)
    # Replace non-breaking spaces with regular spaces (preserves word boundaries)
    s = s.replace("\u00a0", " ")
    # Remove truly invisible characters (zero-width, soft-hyphen, BOM)
    s = re.sub(r"[\u200b\u200c\u200d\u200e\u200f\u00ad\ufeff]+", "", s)
...
def to_title_case_name(v: str) -> str:
    """Title-case a name, preserving hyphens, Mc/Mac/O' prefixes and short all-caps abbreviations."""
    if not v:
        return ""
    v = _clean_string_universal(str(v))
    # Strip stray digits (e.g. 'John2' → 'John')
    v = re.sub(r"\d+", "", v).strip()
    if not v:
        return ""

    def _cap_token(token: str) -> str:
        """Title-case one whitespace-delimited token, handling hyphens and prefixes."""
        # Preserve short all-caps abbreviations: JR, II, III (<=3 chars)
        # 4-char all-caps tokens (e.g. JOHN) should still be title-cased
        if token.isupper() and len(token) <= 3:
            return token
        # Handle hyphenated parts: mary-jane → Mary-Jane
        if "-" in token:
            return "-".join(_cap_part(p) for p in token.split("-"))
        return _cap_part(token)

    def _cap_part(p: str) -> str:
        """Title-case a single part, aware of Mc/Mac/O' prefixes."""
        if not p:
            return ""
        pl = p.lower()
        # O'Brien, O'Sullivan
        ...
        if pl.startswith("mac") and len(p) > 3:
...
# Excel date serial epoch (Windows): 1899-12-30
_EXCEL_EPOCH = datetime(1899, 12, 30)
# Reasonable Excel serial range: 1900-01-01 (1) to 2200-01-01 (~109574)
_EXCEL_MIN = 1
_EXCEL_MAX = 109574


def parse_to_dd_mm_yyyy(raw: str):
    """Parse many date representations to dd-mm-yyyy string. Returns None on failure."""
    if not raw:
        return None
    s = str(raw).strip()

    # Idempotency — already in target format
    ...
    # Excel numeric serial (e.g. 46000)
    if re.match(r"^\d{4,6}$", s):
        n = int(s)
        if _EXCEL_MIN <= n <= _EXCEL_MAX:
            try:
                dt = _EXCEL_EPOCH + timedelta(days=n)
                # Skip plausible years outside 1900-2200
                if 1900 <= dt.year <= 2200:
                    return dt.strftime("%d-%m-%Y")
            except Exception:
                pass
        # Plain 4-6 digit number that isn't a serial — not a date
        return None

    # Strip ordinal suffixes: 1st, 2nd, 3rd, 28th → 1, 2, 3, 28
    ...
    for fmt in numeric_formats:
        try:
            ...
            if "%y" in fmt and dt.year > 2030:
                dt = dt.replace(year=dt.year - 100)
            return dt.strftime("%d-%m-%Y")
        except ValueError:
            continue

    # Text month patterns: "28 Feb 2026", "28 February 2026",
    # "Feb 28 2026", "February 28, 2026", "28-Feb-2026"
    text_month_re = re.compile(
        r"""(?:
            (?P<d1>\d{1,2})\s*[-./,\s]\s*(?P<m1>[a-zA-Z]{3,9})\s*[-./,\s]?\s*(?P<y1>\d{2,4})
            |
            (?P<m2>[a-zA-Z]{3,9})\s*[-./,\s]\s*(?P<d2>\d{1,2})\s*[-./,\s]?\s*,?\s*(?P<y2>\d{2,4})
        )""",
        re.VERBOSE,
    )
    m = text_month_re.search(s_clean)
    if m:
        try:
            if m.group("d1"):
                day, mon_s, year_s = m.group("d1"), m.group("m1"), m.group("y1")
            else:
                day, mon_s, year_s = m.group("d2"), m.group("m2"), m.group("y2")
            mon = _MONTH_MAP.get(mon_s.lower()[:9])
            if mon:
                year = int(year_s)
                if year < 100:
                    year = 2000 + year if year < 30 else 1900 + year
                dt = datetime(year, mon, int(day))
                return dt.strftime("%d-%m-%Y")
...
def clean_numeric_id(raw, required: bool):
    ...
    # Handle float-like values: 1001.0 → '1001'
    if isinstance(raw, float):
        if raw.is_integer():
            raw = int(raw)
        else:
            raw = raw  # let digit extraction handle it
    s = str(raw).strip()
    # If it looks like a float string (e.g. '1001.0'), convert cleanly first
    if re.match(r'^\d+\.0+$', s):
        s = str(int(float(s)))
    digits = re.sub(r"\D+", "", s)
    if digits:
        # Strip leading zeros (e.g. '00123' → '123'), but keep '0' itself
...
def clean_title(raw, required: bool):
    ...
    v = str(raw).strip().lower().replace(".", "").replace(",", "")
    mapping = {
        "mr": "Mr",
        "ms": "Ms",
        "mrs": "Mrs",
...
_TLD_TYPOS = {
    ".cmo": ".com", ".ocm": ".com", ".con": ".com", ".copm": ".com",
    ".og": ".org", ".ogr": ".org",
    ".ent": ".net", ".nte": ".net",
    ".vom": ".com", ".comm": ".com",
}
...
def clean_email(raw, required: bool):
    ...
    v = _clean_string_universal(str(raw)).replace(" ", "").lower()
    # Common obfuscation replacements
    v = v.replace("(at)", "@").replace("[at]", "@").replace(" at ", "@")
    # Strip trailing punctuation (commas, dots appended by sentence)
    v = v.rstrip(".,;")
...
TEST_SYNONYMS = {
    ...
    "drugtest": "Drug Screen",
    "urine": "Drug Screen",
    "urinalysis": "Urinalysis",
    "hearing": "Audiometry",
    "hearingtest": "Audiometry",
}
...
def clean_generic_string(raw, required: bool):
    if raw is None or str(raw).strip() == "":
        return "", ("bad" if required else "ok")
    cleaned = _clean_string_universal(str(raw))
    if not cleaned:
        return "", ("bad" if required else "ok")
    return cleaned, "ok"


def clean_phone(raw, required: bool):
    """Normalise Australian phone numbers. Returns formatted string and status."""
    if raw is None or str(raw).strip() == "":
        return "", ("bad" if required else "ok")

    s = _clean_string_universal(str(raw))
    # Strip everything except digits and leading +
    has_plus = s.startswith("+")
    digits = re.sub(r"\D", "", s)

    # Convert +61 international to local 0X format
    if has_plus and digits.startswith("61") and len(digits) >= 11:
        digits = "0" + digits[2:]
    ...
    # Australian mobile: starts 04, 10 digits → 04XX XXX XXX
    if re.match(r"^04\d{8}$", digits):
        return f"{digits[:4]} {digits[4:7]} {digits[7:]}", "ok"

    # Australian landline: starts 02/03/07/08, 10 digits → 0X XXXX XXXX
    if re.match(r"^0[2378]\d{8}$", digits):
        return f"{digits[:2]} {digits[2:6]} {digits[6:]}", "ok"

    # 8-digit local number (missing area code) — return as-is, ok
    if len(digits) == 8:
        return f"{digits[:4]} {digits[4:]}", "ok"

    # 10+ digit international (non-AU) — return digits only
    if len(digits) >= 10:
        return digits, "ok"
...
def clean_people_value(field, raw):
    ...
    if key in ("phone", "mobile", "telephone"):
        return clean_phone(raw, required)
...
def clean_value(template_key: str, field: dict, raw, valid_ref_ids: dict = None, session_context: list = None, all_templates: dict = None):
    ...
    if status == "ok":
        ...
        # --- RELATIONSHIP FIELD (no FK validation at preview time) ---
        # Any non-empty value is accepted. Relational integrity is enforced
        # at ingestion time, not during the interactive preview.
        if pattern == "relationship":
            s_norm = _normalize_id(s)
            return s_norm if s_norm else clean_val, "ok"

    # --- AI Value-Hint rescue ---
    # If the algorithmic pass still flags this as bad, try the AI-generated regex hint.
    # A regex match upgrades to 'warn' (amber in UI) rather than 'bad' (red) —
    # the value looks plausible but wasn't caught by hardcoded rules.
    if status == "bad" and s not in (None, ""):
        ai_hints = field.get("ai_valueHints") or {}
        hint_regex = ai_hints.get("regex")
        if hint_regex:
            try:
                if re.match(hint_regex, str(s).strip()):
                    status = "warn"
            except re.error:
                pass  # malformed AI regex — ignore silently
                ...
                try:
                    ...
                    try:
                        ...
                        with open(log_path, "a") as f_log:
                            ...
                            f_log.write(f"CONTEXT: {t_key} rows={len(t_rows)} id_field={id_field} ALL_IDS={found_ids}\n")
                            if "1006" in found_ids:
                                f_log.write(f"  -> FOUND 1006 in {t_key} context!\n")
                            else:
                                f_log.write(f"  -> 1006 MISSING from {t_key} context\n")
                    except:
                        pass
...
def clean_rows_for_template(template_key: str, rows: list, all_templates: dict = None, tenant_id: str = None, session_context: list = None, valid_ref_ids: dict = None):
    ...
    Uses a request-scoped cache to avoid re-cleaning identical values.
    """
    tpl = get_template(template_key, all_templates)
    ...
    # Request-scoped LRU cache: (template_key, field_key, repr(raw)) → (clean_val, status)
    # Capped at _CLEAN_CACHE_MAX entries to prevent memory exhaustion on high-cardinality
    # columns (e.g. unique names/IDs in 50k-row files). Oldest entries evicted first.
    _CLEAN_CACHE_MAX = 5_000
    ...
    for row in rows:
        ...
        for field in tpl["fields"]:
            ...
            # Cache key: only cache for simple (non-relationship) fields
            # Relationship fields depend on ref IDs which are context-dependent
            use_cache = field.get("pattern") != "relationship"
            cache_key = (template_key, fkey, repr(raw_val)) if use_cache else None

            if cache_key and cache_key in _cache:
                clean_val, status = _cache[cache_key]
                _cache.move_to_end(cache_key)  # mark as recently used
            else:
                clean_val, status = clean_value(
                    template_key, field, raw_val,
                    valid_ref_ids=valid_ref_ids,
                    session_context=session_context,
                    all_templates=all_templates,
                )
                if cache_key:
                    _cache[cache_key] = (clean_val, status)
                    if len(_cache) > _CLEAN_CACHE_MAX:
...
def waterfall_header_match(
    ...
    for field in tpl["fields"]:
        ...
        # Also add field-level synonyms defined in the template schema
        for syn in (field.get("synonyms") or []):
            field_synonyms_norm.add(normalise_header_string(syn))
        # Also add AI-generated synonyms (injected at template load time by apply_enrichments_to_fields)
        for syn in (field.get("ai_synonyms") or []):
            field_synonyms_norm.add(normalise_header_string(syn))
...
def enhance_rows_with_ai(
    ...
    """
    # Hard cap: only send at most MAX_AI_ERROR_ROWS errored rows to AI.
    # This prevents runaway API costs on large imports (e.g. 50k rows, 10% errors = 5k AI calls).
    # Rows beyond the cap are passed through unchanged with their original (possibly bad) values.
    # The caller receives an `aiCapped` count so the frontend can warn the user.
    # --- Selective cleaning (always-on): only error rows ever go to AI ---
    # Normalise row_errors keys to int (they may arrive as str from JSON)
    ...
    rows_with_errors = [r for r in initial_list if r.get("__rowIndex") in error_indices]
    rows_already_clean = [r for r in initial_list if r.get("__rowIndex") not in error_indices]

    print(f"AI: {len(rows_with_errors)} rows have errors, {len(rows_already_clean)} clean rows skipped.")

    if not rows_with_errors:
        # Nothing to fix — return immediately, no API call
        return initial_list, None

    cleaned_rows = rows_with_errors
    externally_skipped = rows_already_clean


    # --- Apply hard AI row cap ---
    # Sort: error rows first (most important), then by rowIndex
    def get_row_priority(r):
        idx = r.get("__rowIndex")
        has_error = idx in error_indices
    ...
    if len(cleaned_rows) > MAX_AI_ERROR_ROWS:
        ai_capped_count = len(cleaned_rows) - MAX_AI_ERROR_ROWS
        print(
            f"WARN: AI cap hit — sending top {MAX_AI_ERROR_ROWS} error rows to AI, "
            f"{ai_capped_count} rows skipped (no AI applied, original values preserved)."
        )
        rows_to_process = cleaned_rows[:MAX_AI_ERROR_ROWS]
        skipped_rows = cleaned_rows[MAX_AI_ERROR_ROWS:] + externally_skipped
    ...
    ai_usage_summary = {
        ...
        "aiCapped": ai_capped_count,        # rows that hit the cap and were NOT sent to AI
        "maxAiErrorRows": MAX_AI_ERROR_ROWS, # the cap value, for display in the frontend
    }
...
def detect_templates_with_ai(headers: list, sample_data: list, tenant_id: str, all_templates: dict = None) -> list:
    ...
    for k, v in source.items():
        # Build a simplified list of fields to help the AI match headers
        simplified_fields = []
        for f in v.get("fields", []):
            syns = f.get("synonyms", []) + f.get("ai_synonyms", [])
    ...
    prompt_obj = {
        ...
        "instructions": [
            ...
            "Analyze the uploadedHeaders and sampleData to see which candidate templates are a good fit.",
            "Be lenient: use fuzzy matching, semantic similarity, and check the provided 'synonyms' for each field.",
            "If multiple templates are relevant to the data, return all of them.",
            "A template is a match if a significant portion of the uploaded headers map to its fields, even if the names aren't an exact match.",
            "If no template is even remotely relevant, return an empty list."
        ]
    }
...
@app.route("/api/import/presign", methods=["POST"])
def api_presign_upload():
    """
    Generate an S3 pre-signed URL so the frontend can upload massive CSV files
    DIRECTLY to S3, bypassing the Flask server memory entirely.
    """
    try:
        from aws_services import generate_presigned_upload_url
    except ImportError:
        return jsonify({"error": "AWS services not configured"}), 500

    data = request.get_json(force=True, silent=False) or {}
    tenant_id = data.get("tenantId")
    file_name = data.get("fileName")
    file_type = data.get("fileType", "text/csv")

    if not tenant_id or not file_name:
        return jsonify({"error": "tenantId and fileName are required"}), 400

    presigned_data, s3_key = generate_presigned_upload_url(tenant_id, file_name, file_type)
    
    if not presigned_data:
        return jsonify({"error": "Failed to generate presigned URL. Check AWS credentials."}), 500
...
@app.route("/api/import/enqueue-aws", methods=["POST"])
def api_enqueue_aws():
    """
    Called by React after the raw CSV is uploaded to S3 via the presigned URL.
    This creates a Job in MongoDB and enqueues messages to SQS for the worker fleet.
    """
    import uuid
    from datetime import datetime as _dt
    try:
        from aws_services import enqueue_import_job
    except ImportError:
        return jsonify({"error": "AWS services not configured"}), 500
    ...
    file_name = data.get("fileName", "upload.csv")
    templates = data.get("templatesToProcess", [])
    mappings = data.get("mappings", {})
    settings = data.get("settings", {})

    if not tenant_id or not s3_key or not templates:
        return jsonify({"error": "tenantId, s3Key, and templatesToProcess are required"}), 400
    ...
    job_doc = {
        ...
        "tasksTotal": len(templates),
        "tasksCompleted": 0
    }
    
    coll_jobs.insert_one(job_doc)

    # Enqueue a message for each template (worker processes them independently)
    for t_key in templates:
        success = enqueue_import_job(s3_key, t_key, tenant_id, job_id)
        if not success:
            print(f"Failed to enqueue SQS job for {t_key}")

    return jsonify({"jobId": job_id, "status": "enqueued"})
    ...
    def process_single_template_mapping(template_key):
        ...
        for f in tpl["fields"]:
            field_obj = {
                ...
                "description": f.get("description", ""),
                "synonyms": f.get("synonyms", []) + f.get("ai_synonyms", [])
            }
            if f.get("allowed"):
                field_obj["allowedValues"] = f.get("allowed")
...
def api_get_tenants():
    ...
    # Read from MongoDB tenants collection
    tcoll = get_tenants_collection()
    all_tenants = []
    if tcoll is not None:
        for doc in tcoll.find({}, {"_id": 0}):
            all_tenants.append({
                "id": doc.get("tenantId"),
                "name": doc.get("name", doc.get("tenantId")),
            })

    # Super-admin sees all tenants
    is_super = False
    if user:
        is_super = user.get("isSuperAdmin", False) or user_id == "admin-1"
    if is_super:
        return jsonify({"tenants": all_tenants})
    ...
    accessible = [t for t in all_tenants if t["id"] in accessible_ids]
...
def api_clean_values():
    ...
    # --------------- Async threshold ----------------------------------------
    # For files with >= ASYNC_PREVIEW_THRESHOLD rows we run the clean in a
    # background thread and return a jobId immediately so the request doesn't
    # time out.  Small files take the unchanged synchronous path below.
    ASYNC_PREVIEW_THRESHOLD = 5000

    if len(rows) >= ASYNC_PREVIEW_THRESHOLD:
        import uuid, threading
        preview_job_id = f"preview-{uuid.uuid4().hex[:12]}"
        coll_jobs = get_ingestion_jobs_collection()

        if coll_jobs is not None:
        ...
        def _run_async_preview():
            try:
                v_ref_ids = get_valid_ref_ids(template_key, all_templates, tenant_id=tenant_id, session_context=full_dataset_context)
                cr, re_, inf = clean_rows_for_template(
                    template_key, rows, all_templates,
                    tenant_id=tenant_id, session_context=full_dataset_context, valid_ref_ids=v_ref_ids
                )
                out = []
                for r in cr:
                    obj = {"__rowIndex": r["__rowIndex"]}
                ...
                if coll_jobs is not None:
                    coll_jobs.update_one(
                        ...
                        {"$set": {
                            ...
                            "preview_result": {
                                "outputRows":  out,
                                "rowErrors":   {str(k): v for k, v in re_.items()},
                                "inferredIds": inf,
                            }
                        }}
                    )
            except Exception as exc:
                print(f"ASYNC PREVIEW: job {preview_job_id} failed: {exc}")
                if coll_jobs is not None:
                    coll_jobs.update_one(
                        {"jobId": preview_job_id},
        ...
        t = threading.Thread(target=_run_async_preview, daemon=True)
        t.start()
        ...
        return jsonify({
            ...
            "jobId":       preview_job_id,
            "totalRows":   len(rows),
            "threshold":   ASYNC_PREVIEW_THRESHOLD,
            "status":      "pending",
        })
    # -------------------------------------------------------------------------
