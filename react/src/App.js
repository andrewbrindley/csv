...
export default function App() {
  ...
  const handleFiles = async (fileList) => {
    ...
    try {
      ...
      const presignRes = await authFetch(`${API_BASE}/import/presign`, {
        method: "POST",
        body: JSON.stringify({
          tenantId: selectedTenantId,
          fileName: file.name,
      ...
      const { url, s3Key: newS3Key } = presignData;
      setS3Key(newS3Key);
      ...
      // 3. Parse only the first 1MB locally for headers/sample data
      // This prevents the browser from crashing on 500k row files.
      const slice = file.slice(0, 512 * 1024); // 512KB is plenty for sample data
      ...
      // Detected template(s)
      const detectedKey = detectTemplateForHeaders(parsed.headers || []);
      const detectedKeys = detectedKey ? [detectedKey] : [];

      setDetectedTemplateKeys(detectedKeys);
  ...
  const handleAsyncIngestion = async () => {
    ...
    try {
      if (!s3Key) {
        throw new Error("Missing S3 Key. Please re-upload the file.");
      }

      // Collect the defined mappings for the templates we are about to process
      const mappingsForJob = {};
      templatesToProcess.forEach(tKey => {
        const config = mappingsByTemplate[tKey] || {};
        mappingsForJob[tKey] = {
          mapping: config.mapping || {},
          mappingSrc: config.mappingSrc || {}
        };
      });

      console.log("Triggering SQS AWS Import Job...");
      const jobResRaw = await authFetch(`${API_BASE}/import/enqueue-aws`, {
        method: "POST",
        body: JSON.stringify({
          tenantId: selectedTenantId,
          s3Key: s3Key,
          fileName: fileName || "uploaded_file.csv",
          templatesToProcess: templatesToProcess,
          mappings: mappingsForJob,
          useAi: true, // or respect a state toggle
          settings: {
            includePiiInAi,
            fillMissing,
            cleanValidValues
      ...
      if (!jobResRaw.ok) throw new Error("Failed to enqueue AWS job");
      const jobRes = await jobResRaw.json();

      console.log("AWS SQS Job triggered. JobId:", jobRes.jobId);
      setActiveJobId(jobRes.jobId);

      const combinedLabel = templatesToProcess.map(tKey => {
        return templatesByKey[tKey]?.label || tKey;
      }).join(", ");
      ...
      setProcessingSummary({
        ...
        rowCount: "TBD", // Handled by stream
        fileName: fileName || "uploaded_file.csv",
        status: "Pending",
        database: "csv",
