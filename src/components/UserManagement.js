...
export default function UserManagement({ tenantId }) {
    ...
    const handleRemoveUser = async (userId, userName) => {
        if (!window.confirm(`Are you sure you want to remove ${userName || userId} from this tenant?`)) return;

        setError(null);
        setSuccess(null);

        try {
            const res = await authFetch(`${API_BASE}/admin/users?tenantId=${tenantId}&userId=${userId}`, {
                method: "DELETE",
            });
            const data = await res.json();
                                ...
                                <UserRow key={u.userId}>
                                    ...
                                    <Td>
                                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                            ...
                                            <button
                                                onClick={() => handleRemoveUser(u.userId, u.name || u.email)}
                                                style={{
                                                    background: "none", border: "none", color: "#ef4444",
                                                    cursor: "pointer", padding: "4px", display: "flex",
                                                    alignItems: "center", justifyContent: "center",
                                                    opacity: 0.7, hoverObserver: "1"
                                                }}
